...
- `m01_weathered_copper` ships without a normal map (content gap surfaced by
  the render baseline).
- The rotating-key-light GIF preview mode (`render_preview_sweep`) is
  merged but still 🔌 not ✅ pending Grayson's own run through the live MCP
  tool surface, per STATUS.md. Its sweep tuning (frame count 18, 80ms/frame,
  6 settle-frames per step) is a first guess, not visually validated across
  a range of materials -- revisit if the GIF reads too choppy/slow once
  Grayson has used it on a few more.
