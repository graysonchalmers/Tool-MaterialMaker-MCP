...
| 3 | Authoring quality: prompt-to-graph, tune | ≥70% usable-hit-rate (≥11/15, any-variant) on frozen test set | 🔌 | plan: `docs/superpowers/plans/2026-08-26-material-maker-mcp-phase3.md`; 3A harness ✅ verified (see sub-gates), 3B/3C not started |
| 3A | Test set + scoring harness | Harness renders a known case → contact sheet + scorecard, exit 0; 15-case set frozen after Grayson review | 🔌 | harness ✅: `run_case.py` on bricks-as-`s01_red_brick_wall` → 4 maps + `source.ptex` + scorecard, exit 0. Test set: 15 cases authored across 7 categories, **awaiting Grayson review + freeze** |
| 3B | Baseline measurement | Baseline scorecard, all 15 scored + miss taxonomy, recorded | ⬜ | — |
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | ⬜ | — |
