...
_Last updated: 2026-08-27 (overlay builder, Phase 5 spec sub-plan step 1 of 4)_
...
| 5 | Live-control (later) | In-app socket drive, watchable build | 🔌 | Design speced 2026-08-26: `docs/superpowers/specs/2026-08-26-live-control-addon-design.md` (Godot addon + disposable overlay, no source fork). No target date; 2 feasibility risks flagged in the spec, both retired by a spike. Sub-plan has 4 steps (§"Phases and gates"): **1. Overlay builder — ✅ wired/verified.** `overlay.py`'s `ensure_overlay` (build/detect-staleness for a disposable MM-checkout-plus-addon working copy) built via TDD across 5 tasks, then a whole-branch review pass (6 findings, all fixed in one wave: input validation before destructive fs work, non-UTF-8 marker handling, case-insensitive path comparison, docs). 25 overlay-specific tests; full fast suite 134 passed (up from 109 before this sub-plan started). Reviewed at task level (5 task reports) and whole-branch. Steps 2–4 not started: **2.** Addon skeleton (socket server, `ping`/`get_graph`). **3.** Mutating commands (`add_node`/`connect_nodes`/`set_param`/`render`). **4.** MCP tool surface (`live_start`/`live_get_graph`/`live_apply`/`live_render` in `server.py`). |
...
| overlay.py (live-control overlay builder) | 🔌 | Phase 5 spec sub-plan step 1 of 4: `ensure_overlay` builds/refreshes a disposable MM-checkout-plus-addon working copy, pure filesystem, no Godot/Config dependency; `tests/test_overlay.py` (25 tests). Not yet consumed by `server.py` -- steps 2-4 (addon skeleton, mutating commands, MCP tool surface) still not started |
