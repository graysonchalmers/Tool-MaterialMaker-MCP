...
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | 🔌 | **iter1 = 8/15 (53%)**, up from 3/15 baseline. +5 solid via recolor lever (f02 leather, w02 barn wood, m01 copper, s03 concrete) + grain-ramp (w01 oak). Scorecard: `quality/scorecards/2026-08-26-iter1.md`; recipes in `docs/AUTHORING.md`. **Blocked (albedo correct, normal FLAT):** s02 granite, o01 moss, m02 aluminum — from-scratch `normal_map` compound won't produce relief; fix = clone a working example's normal chain (see AUTHORING pitfalls). Reaching those 3 → 11/15 gate. Untouched hard composites: f01 weave, man01/man02 hex, combo01 paint+peel |
