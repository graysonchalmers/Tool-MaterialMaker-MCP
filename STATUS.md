...
| 3 | Authoring quality: prompt-to-graph, tune | ≥70% usable-hit-rate (≥11/15, any-variant) on frozen test set | ✅ | **11/15 (73%) — gate met.** plan: `docs/superpowers/plans/2026-08-26-material-maker-mcp-phase3.md`; 3A+3B+3C all ✅ verified |
...
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | ✅ | **iter1 = 11/15 (73%) — gate met**, up from 3/15 baseline. Levers: recolor (f02, w02, m01, s03) + grain-ramp (w01) + clone-a-sharp-edged-example (o01 moss ← dry_earth) + **voronoi per-cell-random flecks (s02 granite, port 2)** + **wood-grain-as-brush-streaks (m02 aluminum, straightened wood clone)**. s02 audit resolved: was foggy (rock's coarse voronoi), fixed with a fine per-cell fleck source → genuine HIT. m02 fixed by cloning wood's directional-streak-with-working-normal chain. Scorecard: `quality/scorecards/2026-08-26-iter1.md`; recipes in `docs/AUTHORING.md`. **Still MISS (headroom, not blocking):** f01 weave, man01/man02 hex, combo01 paint+peel. |
