...
| 3 | Authoring quality: prompt-to-graph, tune | ≥70% usable-hit-rate (≥11/15, any-variant) on frozen test set | ✅ | **14/15 (93%) — gate met with wide margin.** plan: `docs/superpowers/plans/2026-08-26-material-maker-mcp-phase3.md`; 3A+3B+3C all ✅ verified |
...
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | ✅ | **iter1 = 14/15 (93%) — gate met with wide margin**, up from 3/15 baseline. Levers: recolor (f02, w02, m01, s03) + grain-ramp (w01) + clone-a-sharp-edged-example (o01 moss) + voronoi per-cell-random flecks (s02) + wood-grain-as-brush (m02) + beehive hex clone (man01, man02) + **paint-over-rust peel composite (combo01: flat paint coat blended over rusted_metal through an irregular perlin peel mask, roughness contrast)**. Scorecard: `quality/scorecards/2026-08-26-iter1.md`; recipes in `docs/AUTHORING.md`. **Only MISS: f01 denim weave** — albedo reads as a strong indigo diagonal twill (diagonal_weave grafted into crocodile_skin), but the rubric needs the weave *in the normal* and that donor's `normal_map` renders FLAT headlessly. This is the open **flat-normal blocker (option E)**: `normal_map` emits flat for the crocodile/wood donor family, while dry_earth/bricks donors give rich normals (but dry_earth's warp swirls a clean weave into marbling). 15/15 needs that dig. |
