...
| 3 | Authoring quality: prompt-to-graph, tune | ≥70% usable-hit-rate (≥11/15, any-variant) on frozen test set | ✅ | **13/15 (87%) — gate met with margin.** plan: `docs/superpowers/plans/2026-08-26-material-maker-mcp-phase3.md`; 3A+3B+3C all ✅ verified |
...
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | ✅ | **iter1 = 13/15 (87%) — gate met with margin**, up from 3/15 baseline. Levers: recolor (f02, w02, m01, s03) + grain-ramp (w01) + clone-a-sharp-edged-example (o01 moss ← dry_earth) + **voronoi per-cell-random flecks (s02 granite, port 2)** + **wood-grain-as-brush-streaks (m02 aluminum)** + **beehive hex clone (man01 metal grating + man02 ceramic hex tiles)**. man01 relief carried by the HEIGHTMAP; man02 drives albedo/roughness off the clean hex field (bypassing beehive's per-cell random) for uniform white tiles + thin dark grout. Scorecard: `quality/scorecards/2026-08-26-iter1.md`; recipes in `docs/AUTHORING.md`. **Still MISS (headroom, not blocking):** f01 denim weave, combo01 paint+peel. |
