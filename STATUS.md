...
| 3C | Tuning loop | ≥70% hit-rate scorecard, Grayson-audited | 🔌 | **iter1 = 10/15 (67%)**, up from 3/15 baseline; **1 short of the 11/15 gate**. Solid: recolor lever (f02, w02, m01, s03) + grain-ramp (w01) + clone-a-sharp-edged-example (o01 moss ← dry_earth). s02 granite counted but **FLAGGED for audit** (polished → near-flat normal). Scorecard: `quality/scorecards/2026-08-26-iter1.md`; recipes in `docs/AUTHORING.md`. **Still MISS:** m02 aluminum (needs crisp directional relief), f01 weave, man01/man02 hex, combo01 paint+peel. Landing any ONE reaches the gate (audit s02, or fix m02). |
